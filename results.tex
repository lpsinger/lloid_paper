...
interpolation filter length to 16. Table \ref{table:flops} lists the costs for filtering this sub-bank. Both the \FD\ method and \lloid\ are five orders of magnitude faster than the conventional \TD\ method.  However, the \FD\ method has a latency of over half of an hour, whereas \lloid\ has no latency at all.
%
...
length of templates varies over the parameter space.  Lower mass templates are
longer and would require more computational cost, higher mass templates are
shorter and would require less.  We consider the estimates based on this
sub-bank to be a reasonable representation, however}, an entire search could be
implemented with $\gtrsim 10$ machines.
