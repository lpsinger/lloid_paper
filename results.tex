...
\lloid\ (theory) & $6.6\times10^8$ & $1.2$ \\
\lloid\ (prototype) & ($\sim$0.9 cores) & $1.5$ \\
\tableline
\end{tabular}
\end{center}
\end{table}
...
Table~\ref{table:flops} shows that the \lloid\ method requires $\sim$$10^9$
...
search could be implemented with $\gtrsim$$100$ machines.  Template length does
...
sub-bank to be a reasonable representation because they are within an order of
magnitude of estimates based on the approximate speedup in
...
a total of $\gtrsim$$10^{15}$ \flops\ or $\gtrsim$$10^6$ present-day machines
