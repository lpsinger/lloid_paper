...
\usetikzlibrary{fit,calc,chains,scopes}
\begin{document}
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\foreach \x in {0,1,2} {
				...
				\node[resample, left=1.5cm of in\x tee] (in\x) {\tiny$\downarrow$};
		...
		\end{tikzpicture}
		\caption{Schematic of LLOID pipeline illustrating signal flow.  Circles with arrows represent upsampling \protect\tikz{\protect \node[resample] {\tiny$\uparrow$};} or downsampling \protect\tikz{\protect \node[resample] {\tiny$\downarrow$};}.  Circles with plus signs represent summing junctions \protect\tikz{\protect \node[line width=1pt,adder] {\tiny$\uparrow$};}.  Squares \protect\tikz{\protect \node[line width=1pt,draw] {};} stand for FIR filters.  Sample rate decreases from the top of the diagram to the bottom.}
	\end{center}
\end{figure}
