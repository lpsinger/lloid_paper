...
%
	...
	\node[draw,fill=white,minimum width=22mm,xshift=1mm,label=right:{\small delay}] at (delay1) {\tiny$z^{-t^2 f^1}$};
	\draw[->] (in0) -- (in1);
	\draw (in1) -- (invdots);
	\draw[->] (invdots) -- (in2);
	\draw[->] (x) -- (in0);
	...
	\draw[decorate, decoration={brace, amplitude=4pt}] ($(intee)+(-2mm,0)$) -- ($(u)+(2mm,0)$) node[midway,above=2mm,align=center] {Orthogonal \\ FIR filters};
	...
	\draw[decorate, decoration={brace, amplitude=4pt}] ($(add0)+(-2mm,0)$) -- ($(add3)+(2mm,0)$) node[midway,above=2mm,align=center] {Interpolation and \\ SNR accumulation};
\end{tikzpicture}
