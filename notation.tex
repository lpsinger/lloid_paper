...
\begin{equation}
x^{s+1}[k^{s+1}] = \sum_{n=-N^\shortdownarrow/2}^{N^\shortdownarrow/2-1} b^\shortdownarrow[n] x^{s}\!\left[k^{s} - n\right] \text{, where } k^{s} = k^{s + 1} \frac{f^{s}}{f^{s+1}}
\end{equation}

\textsc{svd}:
\begin{equation}
h^{s}_{i}\!\left[k^{s}\right] = \sum_{m=0}^{M-1} v^{s}_{im} \sigma^{s}_m u^{s}_{m}[k^{s}] \approx \sum_{l=0}^{L^s-1} v^{s}_{il} \sigma^{s}_l u^{s}_{l}[k^{s}] \text{, where } k^{s} = \frac{f}{f^{s}} k
\end{equation}
