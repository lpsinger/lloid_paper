...
\end{equation}
Here, $\mathcal{M}$ is the chirp mass of the binary in units of time (where $G M_\odot / c^3 \approx 5 \mu\mathrm{s}$) and $t$ is the time remaining until the coalescence of the binary~\cite{findchirppaper, kidder1992, blanchet2002, hanna2009}. \editorial{Way too many citations here.} Usually the template is truncated at some prescribed time $t = t_0$ before coalescence, often chosen to correspond to the innermost stable circular orbit at which $f = f_\mathrm{ISCO} = 4400 \, M_\odot / M$.  The full template will extend to some low frequency cutoff, $f = f_\mathrm{low}$ corresponding to a time $t_\mathrm{low}$.  This template has a duration of $t_\mathrm{low} - t_\mathrm{0}$ and is critically sampled at a rate of $2 f_\mathrm{isco}$.
...
If all of these time slices are implemented using the \textsc{fft}, then the latency of this filter bank is $\max_j \left( 2(t_j - t_{j-1}) - t_{j-1} \right) = \max_j \left( 2 t_j - 3 t_{j-1} \right)$.
