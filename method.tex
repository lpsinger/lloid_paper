...
%
\editorial{Drew: is ``sample count" the correct wording?}
...
matched filter approach.  In the remainder of this section 
%
\editorial{Drew: We used this wording at the beginning of this paragraph.}
%
we describe the
...
\emph{time slices} 
%
\editorial{Drew: This was introduced in the previous section. It should be italicized there.}
%
in a time-domain analogue to the frequency-domain
...
\end{indented}
\end{table}
\editorial{Drew: Is \slicessamps before or after decimation? We should clarify that here.}

\editorial{This section's two tables are small.  Any way to put them side by side or next to another figure?}

\Table{\label{table:flops}Computational cost in \flops\ of the \TD\ method, the \FD\ method, and \lloid\ for the bank of $2 \times 657$ templates sampled at 4096 Hz for chirp masses $1.955 \leqslant \mathcal{M} / M_\odot \leqslant 1.2045$.  A block size of $D = 2 N$ is used for the \fft{}s in the \FD\ method.  Both the \FD\ method and \lloid\ are five orders of magnitude faster than the conventional \TD\ method.  However, the \FD\ method has a latency of over half of an hour, whereas \lloid\ has no latency at all.}
\br
...
\end{indented}
\end{table}
\editorial{Drew:``$1.955 \leqslant \mathcal{M} / M_\odot \leqslant 1.2045$" can't be true. Should the signs be greater than signs?}
...
\editorial{Drew: I think this section be the last section of the results section. Should we move it to be Sec. 4.2?}
