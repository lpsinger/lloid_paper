...
search with minutes latency in \LIGO{}'s S6 and Virgo's VSR3 science runs.
...
Equation~(\ref{eq:SNRTD}) can be implemented in the time domain as an \fir{} filter, requiring $\mathcal O(\numtmps \tmpsamps)$ floating point operations per sample.  However, it is typically much more computationally efficient to use the convolution theorem and the \fft{} to implement fast convolution, requiring only $\mathcal O(\numtmps \lg \tmpsamps)$ operations per sample.
...
template is truncated at some prescribed time $t^0$, or equivalently frequency $f_\mathrm{hi}$.
...
power-of-two sample rate that sub-critically samples the time sliced
...
\editorial{Chad: This whole subsection needs a lot of work.
I just dropped our new notation in here.}%
...
procedure for producing the decomposition in \eqref{eq:decomp} in the remainder
of this section.
