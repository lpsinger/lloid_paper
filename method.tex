...
space that reduces time-domain (\TD) filtering cost sufficiently to allow for the
...
computationally cheap. Our method, \lloid\footnote{\lloid, The term strictly
means \emph{Low Latency Online Inspiral Detection} and was a code name for
this procedure as it was being developed. It is now known in some GW circles.
We feel that although the name doesn't carry much specific information about
the details of the algorithm it is far too late to do anything about it now.}
...
we describe how we form our novel detection statistic using the time-slice
