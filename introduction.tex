...
bursts~\cite{Lee:2005, nakar07}. This prompt emission is a probe into the
extreme initial conditions of the outflow. A very bright optical flash should
accompany the x-rays and gamma-rays~\cite{Sari99}. \editorial{Should mention
that optical flashes have been seen for a handful of long GRBs by very fast
slews. TAROT has seen many within ~20 seconds. \cite{Klotz09}} In contrast, an
...
\emph{after} the merger, but seconds \emph{before}.  \editorial{I think we
should say also something about how we can also get the full SNR within moments
after the coalescence, it is obvious maybe, but we should be explicit}  By
looking for threshold crossings before the signal leaves the detection band, it
is possible to trade some signal to noise ratio (\textsc{snr}) for latency.
Figure \ref{fig:earlywarning} shows projected early trigger rates for NS--NS
binaries in Advanced \LIGO\ assuming the event rate predictions in
