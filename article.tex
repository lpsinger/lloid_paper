...
%
$$
\textrm{signal to noise} = \frac{\expectation \left[ y[0] \right]}{\var \left[ y[k] \right]} = \frac{y_s[0]}{\var \left[ y_n[k] \right]}.
$$
%
...
$$
%
up to an arbitrary multiplicative constant.  Here, $\tilde{h}[n]$, $\tilde{s}[n]$, and $\tilde{x}[n]$ are the discrete Fourier transforms (DFTs) of $h[k]$, $s[k]$, and $x[k]$, respectively; $\tilde{S}[n]$ is the power spectral density of $n[k]$.  The discrete Fourier transform of the output is
%
\begin{multline}
