...
from the plane containing the detectors and worst at zero elevation.  \citeauthor{Fairhurst2009} also cautions that his Fisher matrix calculation fails to capture disconnected patches of probability, which occur prominently in networks of three detectors where there are generally two local maxima on opposite sides of the plane of the detectors.  (\citeauthor{Fairhurst2009} also points out that even his approximation that the timing errors are Gaussian breaks down at low SNR.)  At most sky locations, the localization uncertainty estimated from timing is highly optimistic and will only suffice for an order of  magnitude estimate.  Once per year, we expect to observe an
