...
this problem, a computationally inexpensive low-latency filtering algorithm for
detecting inspiral signals in \GW\ data.  We also consider the prospects for
advanced \GW\ detectors and discuss other areas of work that would be required
for rapid analysis.
...
for compact binary coalescence.  We give a real-time algorithm
...
without making the median mismatch higher than 0.003.

During testing, we were surprised that careful design of the decimation and
interpolation stages made a crucial difference in terms of computational
overhead.  When we tested the pipeline with the largest interpolation filters
provided by GStreamer's \texttt{audioresample} element, we found that rather than
making a negligible contribution to the total \flops\ budget and total measured
overhead they became the dominant contribution.  There is possibly further room
for optimization beyond minimizing $N^\shortuparrow$.  We have considered
designing our own custom decimation and interpolation filters or tuning these
filters separately for each time slice.  We now appreciate that the efficiency
of \lloid\ hinges on proper specification of $N^\shortuparrow$.
...
\lloid\ (theory) & $6.6\times10^8$ & 0 & $\phantom{5.7}\sim 10^{10}$ & $\sim$$10$ \\
\lloid\ (prototype) & (0.9 cores) & $0.5$ & ------------ & $\sim$$10$ \\
\tableline
\end{tabular}
\end{center}
\end{table*}
...
latency free, real-time operation.  We anticipate requiring $\sim$40
