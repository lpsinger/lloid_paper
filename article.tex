...
\altaffiltext{10}{Leibniz Universit\"{a}t Hannover, Hannover, Germany}
\altaffiltext{11}{Ryerson University, Toronto, ON, Canada}
...
from the plane containing the detectors and worst at zero elevation. As
\citeauthor{Fairhurst2009} cautions, his formula also slightly
\emph{overestimates} the minimum area for low SNR events because it
makes an approximation that the timing errors are Gaussian distributed
and that the sky location estimates are Gaussian distributed about the
true location.  Nonetheless, given the large uncertainties in rates,
\citeauthor{Fairhurst2009}'s approximations suffice for an order of
magnitude estimate.
...
$$
%
where $y_s$ is the response of the filter to the signal alone and $y_n$ is the response of the signal to noise alone.  The matched filter's coefficients maximize the ratio of the expectation of the filter's instantaneous response to the variance in the filter's output at all times:
%
$$
\textrm{signal to noise} = \frac{\expectation \left[ y[0] \right]}{\var \left[ y[k] \right]} = \frac{y_s[h]}{\var \left[ y_n[k] \right]}.
$$
%
...
$$
%
up to an arbitrary multiplicative constant.  $\tilde{h}[n]$, $\tilde{s}[n]$, and $\tilde{x}[n]$ are the discrete Fourier transforms (DFTs) of $h[k]$, $s[k]$, and $x[k]$, respectively, here $\tilde{S}[n]$ is the power spectral density of $n[k]$.  The discrete Fourier transform of the output is
%
\begin{multline}
...
slices in a \TD\ analogue to the \FD\ decomposition employed by MBTA~\citep{Marion2004,Buskulic2010}.  The application to GW data analysis is foreshadowed by an earlier \FD\ convolution algorithm based on splitting the impulse response of a filter into smaller blocks, proposed by \citet{gardner1995efficient}.  We decompose each template
